...
public class UmsAdminController {
    ...
    /**
     * 登陆成功后用户信息以jwt令牌的方式返回给前端
     * @param umsAdminLoginParam
     * @param result
     * @return
     *
     * 1.如果前端发的是json字符串，要封装对象
     * @RequestBody UmsAdminLoginParam umsAdminLoginParam
     *
     * 2.如果前端发过来的是k=v&&k==v字符串，不要封装对象
    ...
    public Object getAdminInfo(HttpServletRequest request) {
        ...
        //查询用户的头像
        //1 getOne 是mybatis-plus 生成的，而且带了泛型的。
        //2 dubbo 没办法直接调用mp中带泛型的service
        //3 实战经验 ：
        // 自动生成的有可能有兼容问题
//        Admin umsAdmin = adminService.getOne(new QueryWrapper<Admin>().eq("username",userName));
        ...
        String userIcon = umsAdmin.getIcon();
        Map<String, Object> data = new HashMap<>();
        data.put("username", userName);
        data.put("roles", new String[]{"TEST"});
        data.put("icon", userIcon);
